...
TELEGRAM_BOT_TOKEN = "8128477326:AAG2GY8Y2MLk1_OIvUaGQpUv0lhIUcQOQUM"
...
# ------------------ INITIALIZE BOT Safely & Clean Webhooks ------------------
try:
    bot = telebot.TeleBot(TELEGRAM_BOT_TOKEN, threaded=False)
    print("🧹 Cleaning existing active webhooks to bypass Conflict 409...")
    bot.delete_webhook(drop_pending_updates=True)
    print("✅ Webhook dropped successfully. Ready for Polling Engine.")
...
if __name__ == "__main__":
    ...
    # Secure looping wrapper with explicit safeguards
    while True:
        try:
            bot.infinity_polling(timeout=20, long_polling_timeout=10)
        except telebot.apihelper.ApiTelegramException as te:
            if te.error_code == 409:
                print("⚠️ Webhook conflict intercepted. Retrying deletion parameters...")
                try:
                    bot.delete_webhook(drop_pending_updates=True)
                except:
                    pass
