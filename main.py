# ╔══════════════════════════════════════════════════════════════╗
# ║     Alpha  OTP Bot — Full System Bot                         ║
# ║     Numbers + OTP Forwarding + Full Admin Suite             ║
# ║     Developed by Alpha (@huh_insane7)                    ║
# ╚══════════════════════════════════════════════════════════════╝
...
ADMIN_IDS      = [7064636445, 8222195948]
OTP_GROUP_LINK = "https://t.me/pantherotpgroup"
BOT_NAME       = "PANTHER OTP"
...
def load_config():
    return load_json(CONFIG_FILE, {
        ...
        "number_bot_link": "https://t.me/testing_time6282_bot",
        "otp_forward":     True,
        "forward_delay":   0,
        "log_group":       None,
    })
...
def format_otp_message(number: str, service: str, otp: str,
    ...
    # Format: #WS #DE🇩🇪 +49--ALPHA--72982 #English
    return (
        f"<b>#{svc} #{region}</b>{flag} "
        f"<code>{dial}--PRIME ZONE--{last5}</code> "
        f"<b>#{lang}</b>"
    )
...
async def send_otp_to_owner(number: str, service: str, otp: str,
    ...
    kb = InlineKeyboardMarkup([
        [InlineKeyboardButton(f"🔑 {fmt_otp}", copy_text=CopyTextButton(text=clean_otp), api_kwargs={"style": "primary"})],
        [InlineKeyboardButton("📞 Numbers", url="https://t.me/panthernumbers", api_kwargs={"style": "success"}),
            InlineKeyboardButton("💬 Channel", url="https://t.me/meThod5527", api_kwargs={"style": "danger"})
        ]
    ])
    bot = Bot(token=BOT_TOKEN)
    try:
...
def get_main_menu_keyboard():
    return InlineKeyboardMarkup([
        [b("🧇 𝗚𝗲𝘁 𝗡𝘂𝗺𝗯𝗲𝗿", "show_countries", style="danger"),  b("🫁 𝗣𝗿𝗼𝗳𝗶𝗹𝗲", "user_profile", style="primary")],
        [b("🧠 𝗗𝗲𝘃𝗲𝗹𝗼𝗽𝗲𝗿", url="t.me/huh_insane7", style="success")],
    ])
...
def get_join_keyboard():
    return InlineKeyboardMarkup([
        ...
        [b("📌 Backup Channel",  url="https://t.me/meThod5527")],
